<template>
  <div class="app-container">
    ...
    <!-- 工具条 -->
    <div class="tools-div">
      <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="add">添加用户</el-button>
      <el-button type="danger" plain icon="el-icon-circle-close" size="mini" @click="batchCancelAssignUser"
        :disabled="multiple">取消授权</el-button>
    ...
    <el-table v-loading="listLoading" :data="list" style="width: 100%;margin-top: 10px;" 
      ...
      <el-table-column label="状态" align="center" width="100">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.status == 1" size="small" type="success">{{ scope.row.statusName }}</el-tag>
          <el-tag v-else="scope.row.status == 0" size="small" type="danger">{{ scope.row.statusName }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" fixed="right" width="180">
        <template slot-scope="scope">
          <el-button type="text" icon="el-icon-circle-close" size="mini"
            @click="cancelAssignUserById(scope.row.id)">取消授权</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 弹出框 -->
    <selectUser v-if="dialogVisible" ref="addDialog"></selectUser>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fetchData(page = 1) {
      this.page = page
      api.getSysUserByRoleId(this.roleId, this.page, this.limit, this.searchObj).then(resp => {
    ...
    add() {
      ...
      this.$nextTick(() => {
        this.$refs.addDialog.init(this.roleId);          
      })
    },
    
    // 根据id取消授权
    cancelAssignUserById(id) {
      this.$myconfirm('确认要取消该员工的授权吗？').then(() => {
        return api.cancelAssignUserById(this.roleId, id)
      }).then(resp => {
        this.$message.success("取消授权成功");
        this.fetchData()
      }).catch(error => {
        this.$message.success("取消授权失败");
      })
    },
    // 批量取消授权
    batchCancelAssignUser() {
      if (this.userIds.length == 0) {
        this.$message.warning('请选择要取消授权的记录！')
        return
      }
      this.$myconfirm('确认要取消已勾选员工的授权吗？').then(() => {
        return api.batchCancelAssignUser(this.roleId, this.userIds)
      }).then(resp => {
        this.$message.success("取消授权成功");
        this.fetchData()
      }).catch(error => {
